(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))i(s);new MutationObserver(s=>{for(const a of s)if(a.type==="childList")for(const o of a.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&i(o)}).observe(document,{childList:!0,subtree:!0});function t(s){const a={};return s.integrity&&(a.integrity=s.integrity),s.referrerPolicy&&(a.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?a.credentials="include":s.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function i(s){if(s.ep)return;s.ep=!0;const a=t(s);fetch(s.href,a)}})();class S extends HTMLElement{connectedCallback(){this.innerHTML=`
        ...
        `}}customElements.define("issue-panel",R);class O extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this._rendered=!1,this._expandedStage=null,this._stages=[{key:"initiation",label:"Initiation",status:"pending"},{key:"diagnosis",label:"Diagnosis",status:"pending"},{key:"troubleshoot",label:"Troubleshoot",status:"pending"},{key:"resolution",label:"Resolution",status:"pending"}],this._checkpoints=[]}connectedCallback(){this._rendered||(this.render(),this._rendered=!0)}updateFromState(e){if(!e)return;const t=e.stage||"";this._checkpoints=e.checkpoints||[];const i=["initiation","diagnosis","troubleshoot","resolution"],s=i.indexOf(t.toLowerCase());this._stages=this._stages.map((a,o)=>{let n="pending";return o<s?n="complete":o===s&&(n="active"),{...a,status:n}}),s>=0&&(this._expandedStage=i[s]);for(const a of this._stages){const o=this._checkpoints.filter(n=>n.stage?.toLowerCase()===a.key);o.length>0&&o.every(n=>n.status==="skipped")&&a.status==="complete"&&(a.status="skipped")}this.renderPipeline()}_handleStageClick(e){this._expandedStage=this._expandedStage===e?null:e,this.renderPipeline()}renderPipeline(){const e=this.shadowRoot.querySelector(".rail"),t=this.shadowRoot.querySelector(".checkpoint-details");if(!e||!t)return;e.innerHTML=this._stages.map((a,o)=>{const n=o===this._stages.length-1;n||this._stages[o+1].status;const r=a.status==="complete"||a.status==="skipped"?"filled":"",c=this._expandedStage===a.key,d=this._checkpoints.filter(g=>g.stage?.toLowerCase()===a.key),p=d.filter(g=>g.status==="complete").length;return`
                <div class="step">
        ...
        `,this._updateVisibility()}}customElements.define("agent-guidance",M);const z=`You are the Virtual Internal Assistant for OmniD3sk. You help users navigate government portals, visa applications, tax filing systems, and online services. You are relentless, thorough, and you NEVER close a case with missing information.
...
After you finish speaking, STOP. One turn = one response = then silence. WAIT for the user.`,q={search_knowledge_base:{label:"KB Search",color:"#4d9ff7",icon:"🔍"},lookup_error_code:{label:"Error Lookup",color:"#e57373",icon:"⚠"},lookup_portal_page:{label:"Page Lookup",color:"#ffb74d",icon:"📋"},diagnose_issue:{label:"Diagnosis",color:"#ba68c8",icon:"🔬"},create_issue:{label:"Issue Logged",color:"#ff8a65",icon:"📌"},create_itsm_ticket:{label:"Ticket Created",color:"#81c784",icon:"🎫"},update_itsm_ticket:{label:"Ticket Updated",color:"#81c784",icon:"✏"},research_support_topic:{label:"Web Research",color:"#4dd0e1",icon:"🌐"}};class N extends HTMLElement{constructor(){super(),this.geminiClient=null,this.audioStreamer=null,this.audioPlayer=null,this.screenCapture=null,this._isSessionConnected=!1,this.isScreenSharing=!1,this.isSpeaking=!1,this.sessionToken=null,this._timerInterval=null,this._sessionStartTime=null,this._cmdToastTimeout=null,this._currentPriority=null,this._toolCallCount=0,this._pendingUserTranscript="",this._pendingModelTranscript="",this._panelOpen=!1,this._activeTab="activity",this._toolEntries=[],this._logEntries=[],this._summaryPoints=[],this._currentSentiment="neutral",this._sentimentHistory=[],this._detectedInfo={user:null,module:null,tcode:null,error:null,issue:null}}connectedCallback(){this.innerHTML=`
        ...
        `,this.bindEvents()}disconnectedCallback(){this.cleanup(),this._pasteHandler&&document.removeEventListener("paste",this._pasteHandler)}bindEvents(){const e=this.querySelector("#back-btn"),t=this.querySelector("#mic-btn"),i=this.querySelector("#screen-share-btn"),s=this.querySelector("#screenshot-btn"),a=this.querySelector("#file-input"),o=this.querySelector("#connection-status");e.addEventListener("click",()=>{this.cleanup(),this.dispatchEvent(new CustomEvent("navigate",{bubbles:!0,detail:{view:"home"}}))}),t.addEventListener("click",async()=>{this.isSpeaking=!this.isSpeaking,this.isSpeaking?(t.classList.add("active"),t.innerHTML='<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><rect x="6" y="6" width="12" height="12" rx="1"/></svg>',await this.startSession(o)):(t.classList.remove("active"),this.endSession())}),i.addEventListener("click",()=>this.toggleScreenShare()),s.addEventListener("click",()=>a.click()),a.addEventListener("change",l=>this.handleScreenshotUpload(l)),this.querySelector("#mute-btn").addEventListener("click",()=>this.toggleMute());const n=this.querySelector("#chat-input"),r=this.querySelector("#chat-send-btn"),c=()=>{const l=n.value.trim();!l||!this.geminiClient?.connected||(this.geminiClient.sendTextMessage(l),this.querySelector("#transcript").addInputTranscript(l,!0),this._addTranscriptEntry("user",l,this._elapsed()),this._extractInfo(l,"user"),this._analyzeSentiment(l),this._addLogEntry("ws",`<span class="hl">CHAT_SENT</span> <span class="val">${this._escapeForLog(l)}</span>`),n.value="")};n.addEventListener("keydown",l=>{l.key==="Enter"&&!l.shiftKey&&(l.preventDefault(),c())}),r.addEventListener("click",c);const d=l=>{this.geminiClient?.connected&&(this.geminiClient.sendTextMessage(l),this.querySelector("#transcript").addInputTranscript(l,!0),this._addTranscriptEntry("user",l,this._elapsed()),this._extractInfo(l,"user"),this._analyzeSentiment(l),this._addLogEntry("ws",`<span class="hl">CHAT_SENT</span> <span class="val">${this._escapeForLog(l)}</span>`))},p=this.querySelector("#qa-notion");p&&p.addEventListener("click",()=>d("Please save this current threat report to my Notion workspace. Once it is saved, you MUST reply with the exact `notion_url` text provided by the tool response. Do not use markdown link formatting, just output the raw URL string so I can clearly read it."));const g=this.querySelector("#qa-calendar");g&&g.addEventListener("click",()=>d("Please schedule a follow-up meeting on my calendar for this incident. But FIRST, ask me what time I would like the meeting to be scheduled for and wait for my response. Do not schedule it yet. Once I confirm a time and you have successfully scheduled it, reply with the exact raw Google Calendar link from the tool."));const b=this.querySelector("#qa-discord");b&&b.addEventListener("click",l=>{const u=l.currentTarget,f=u.innerHTML;u.innerHTML='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 8v8"/><path d="M8 12h8"/></svg> Coming Soon',setTimeout(()=>u.innerHTML=f,1500)}),this.querySelector("#toggle-activity-btn").addEventListener("click",()=>this._togglePanel("activity")),this.querySelector("#toggle-insights-btn").addEventListener("click",()=>this._togglePanel("insights")),this.querySelector("#toggle-transcript-btn").addEventListener("click",()=>this._togglePanel("transcript")),this.querySelector("#toggle-logs-btn").addEventListener("click",()=>this._togglePanel("logs")),this.querySelectorAll(".m-tab").forEach(l=>{l.addEventListener("click",()=>this._switchTab(l.dataset.tab))}),this._pasteHandler=l=>{if(!this._isSessionConnected)return;const u=l.clipboardData?.items;if(u){for(const f of u)if(f.type.startsWith("image/")){l.preventDefault();const w=f.getAsFile(),v=new FileReader;v.onload=y=>{const k=y.target.result.split(",")[1];this._sendImageToServer(k),this._showImagePreview(y.target.result)},v.readAsDataURL(w);break}}},document.addEventListener("paste",this._pasteHandler)}_togglePanel(e){const t=this.querySelector("#side-panel");this._panelOpen&&this._activeTab===e?(t.classList.remove("open"),this._panelOpen=!1):(t.classList.add("open"),this._panelOpen=!0,this._switchTab(e))}_switchTab(e){this._activeTab=e,this.querySelectorAll(".m-tab").forEach(t=>t.classList.toggle("active",t.dataset.tab===e)),this.querySelectorAll(".m-panel-section").forEach(t=>t.classList.remove("active")),this.querySelector(`#tab-${e}`)?.classList.add("active")}_addToolEntry(e,t,i,s,a){this._toolCallCount++;const o=this.querySelector("#tool-badge");o.textContent=this._toolCallCount,o.classList.add("visible");const n=this.querySelector("#tab-activity"),r=this.querySelector("#activity-empty");r&&r.remove();const c=JSON.stringify(t||{}).slice(0,120);let d="";if(i)try{const u=typeof i=="string"?JSON.parse(i):i;u.title?d=u.title:u.ticket_id?d=`Ticket ${u.ticket_id}`:u.source_count?d=`${u.source_count} sources found`:d=JSON.stringify(u).slice(0,60)}catch{d=String(i).slice(0,60)}const p=document.createElement("div");p.className="m-tool-entry",p.innerHTML=`
